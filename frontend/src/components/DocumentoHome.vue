  ...
  computed: {
    ...
    filtroDeBusqueda() {
      return this.documentos
        .filter((documento) => documento.titulo.toLowerCase().includes(this.busquedaTitulo.toLowerCase()) &&
  ...
  <div>
    <div class="row">
      <div class="row">
        ...
        <div class="col-md-4 mb-3">
          ...
          <input v-model="busquedaAutor"  type="text" placeholder="Buscar por Autor (introduzca el Autor)"   class="form-control mb-3"/>
        </div>
        <div class="col-md-4 mb-3">
          <h5>Buscar por Sinópsis</h5>
          <input v-model="busquedaSinopsis" type="text" placeholder="Buscar por Sinopsis (introduzca palabras clave de la Sinopsis)" class="form-control mb-3"/>
        </div>
      </div>
    </div>
    <div v-for="documento in filtroDeBusqueda" :key="documento.id" class="card m-0 p-0">
      ...
      <div class="card-body">
        ...
        <p>
          <span>Sinopsis: </span>{{ documento.sinopsis }}
        </p>
        <p>
          <span>Estantería: </span>{{ documento.estanteria }}
        </p>
        <p>
          <span>Fecha alta: </span>{{ formatDate(documento.fechaAlta) }}
        </p>
        <p>
